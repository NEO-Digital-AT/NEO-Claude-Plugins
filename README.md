...
| `neo-google-ads` | Google Ads lesen **und schreiben**: Konten, Kampagnen, Keywords, ausschließende Keywords, Suchbegriffe, Budgets, Gebote, Keyword-Planer. Schreiben ist ein Verfahren, kein Aufruf — messen, Plan vorlegen, Freigabe abwarten, Trockenlauf, umsetzen, belegen; vier Schutzgrenzen darunter, jeder Versuch im Änderungsprotokoll | Mitgelieferter MCP-Server mit dreizehn Werkzeugen, Skill mit sieben Referenzdateien, vier Werkzeugen, Fachagent `ads-betreuung`, Befehle `/neo-google-ads:neo-adseinrichtung`, `/neo-google-ads:neo-adsanalyse` und `/neo-google-ads:neo-adsoptimierung` |
...
| `google-ads-mcp.py` | `plugins/neo-google-ads/scripts/` | MCP-Server für Google Ads über die REST-Schnittstelle der Fassung v25. Dreizehn Werkzeuge: sechs lesen (Konten, siebzehn vorbereitete Berichte, freie GAQL-Abfrage, Feldkatalog, Keyword-Planer, Suchvolumen), sechs schreiben (Keywords, ausschließende Keywords, Status, Budget, Gebote, roher Mutate-Endpunkt), eines zeigt das Änderungsprotokoll. Jeder Schreibaufruf ist zuerst ein Trockenlauf gegen Googles Regelprüfung und verändert nichts. Spricht beide MCP-Fassungen — `initialize` und `server/discover`. Ohne Abhängigkeiten. |
| `google-ads-auth.py` | `plugins/neo-google-ads/scripts/` | Verbindet ein Google-Konto: fragt die vier von Google verlangten Angaben ab, führt den OAuth-Loopback mit PKCE durch, tauscht den Code gegen einen Refresh Token und legt alles mit Rechten 0600 außerhalb des Repositorys ab. `--paste-url` für Maschinen ohne Browser, `--allow-write` setzt die vier Schutzgrenzen, `--show` zeigt den Stand ohne die Geheimnisse, `--env` gibt sie als Übergabeblock für eine Cloud-Sitzung oder eine CI aus — nach Rückfrage, weil er Klartext enthält. Ohne Abhängigkeiten. |
| `google-ads-check.py` | `plugins/neo-google-ads/scripts/` | Misst die Verbindung in sieben Prüfungen — Python, Konfiguration samt Dateirechten, Refresh Token, Developer Token, lesbare Konten, Zustand der Schutzgrenzen — und mit `--customer-id` als achte einen Trockenlauf gegen das echte Konto, der nichts verändert. Jede fehlgeschlagene Prüfung nennt die Abhilfe. Ohne Abhängigkeiten. |
| `google-ads-selftest.py` | `plugins/neo-google-ads/scripts/` | Weist ohne Netz und ohne Zugangsdaten nach, dass die Handbremse hält: 34 Fälle in sechs Gruppen — Schutzgrenzen (Schalter aus, fremdes Konto, Budgetdeckel, Steigerungsfaktor, Operationsgrenze, und der saubere Fall, der durchgehen **muss**), Übersetzung von Googles verschachtelter Fehlerhülle, Aufbereitung der Antworten, GAQL der vorbereiteten Berichte, MCP-Protokoll und Änderungsprotokoll. Gegen vier absichtlich sabotierte Fassungen geprüft; jede fiel auf. Ohne Abhängigkeiten. |
